<?php
	...
	class Model {
		...
		public function get($column='*') {
			...
			// Execute SELECT
			$records = $this->execute_query('SELECT '.$select.' FROM '.static::$table_name.' '.$this->current_query());
		...
		public function count($column='*') {
			$result = $this->execute_query('SELECT COUNT('.$column.') FROM (SELECT * FROM '.static::$table_name.' '.$this->current_query().') A');
		...
		public static function create($params) {
			...
			if ($temp->callback('callback_before_validate') && $temp->validate() && 
				...
				// The Create query
				$temp->execute_query('INSERT INTO '.static::$table_name.' ('.implode(',', $properties).') VALUES ('.$temp_params.')');
		...
		public function update($params) {
			...
			if ($this->query != '') {
				$records = $this->execute_query('SELECT * FROM '.static::$table_name.' '.$this->current_query());
			...
			if (!empty($validated) && $temp_params != '')
				$this->execute_query('UPDATE '.static::$table_name.' SET '.rtrim($temp_params, ',').' WHERE id IN ('.implode(',', $validated).')');
		...
		public function delete() {
			...
			// The Delete query
			if (!empty($validated))
				$records = $this->execute_query('DELETE FROM '.static::$table_name.' WHERE id IN ('.implode(',', $validated).')');
		...
		public function belongs_to($model, $foreign='') {
			...
			$records = $this->execute_query('SELECT '.$foreign.' FROM '.static::$table_name.' '.$this->current_query());		
		...
		public function has_many($model, $foreign='') {
			...
			// Execute the query finding all related objects
			$records = $this->execute_query('SELECT id FROM '.static::$table_name.' '.$this->current_query());
		...
		private function callback($name) {
			if (method_exists($this, $name))
				return $this->name() != FALSE || $this->name() === NULL;
			else 
				return TRUE;
		}
